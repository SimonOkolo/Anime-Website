...
async function getEpSlider(total) {
    ...
    for (let i = 0; i < total.length; i++) {
        ...
        ephtml += `<div class=ep-slide><a href="./episode.php?anime=${x[0]}&episode=${x[1]}"><img onerror="retryImageLoad(this)" class="lzy_img" src="./static/loading1.gif" data-src=https://thumb.anime-dex.workers.dev/thumb/${episodeId}><div class=ep-title><span>Episode ${epNum}</span></div></a></div>`;
    }
...
async function getEpList(total) {
    ...
    for (let i = 0; i < total.length; i++) {
        ...
        ephtml += `<a class="ep-btn" href="./episode.php?anime=${x[0]}&episode=${x[1]}">${x[1]}</a>`;
    }
